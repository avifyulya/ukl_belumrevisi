...
import com.example.ukl_kasir_sekian.databinding.RvCardDatauserBinding
...
class AdapterUser(private val context: Context, private val user : ArrayList<User>, private val userViewModel: UserViewModel): RecyclerView.Adapter<AdapterUser.ViewHolder>() {
    inner class ViewHolder(private val binding : RvCardDatauserBinding): RecyclerView.ViewHolder(binding.root){
        fun bind(user: User){
            ...
            when(user.image){
                1 -> binding.cardUserIvImage.setImageResource(R.drawable.jaehyun)

                2 -> binding.cardUserIvImage.setImageResource(R.drawable.jaemin)

                3 -> binding.cardUserIvImage.setImageResource(R.drawable.nichol)
