...
import com.example.ukl_kasir_sekian.databinding.RvCardMakananBinding
...
class AdapterFood(private val context: Context, private val foods : ArrayList<Food>, private val foodViewModel: MenuViewModel): RecyclerView.Adapter<AdapterFood.ViewHolder>() {
    inner class ViewHolder(private val binding : RvCardMakananBinding): RecyclerView.ViewHolder(binding.root) {
        fun bind(food: Food){
            ...
            when(food.image){
                1 -> binding.cardMakananIvImage.setImageResource(R.drawable.steak_beef)

                2 -> binding.cardMakananIvImage.setImageResource(R.drawable.chicken_salad)

                3 -> binding.cardMakananIvImage.setImageResource(R.drawable.spaghetti)
